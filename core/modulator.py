...
class Modulator(pl.LightningModule):
    def __init__(self, params):
        ...
        self.Lxm = self.params['Lxm']
        self.Lym = self.params['Lym']
        self.Nxm = self.params['Nxm']
        self.Nym = self.params['Nym']
    ...
    def init_phase(self):
        flag = self.phase_initialization
        # Random values [0,2pi]
        if self.phase_initialization == 2:
    ...
    def forward(self, wavefront):
        if wavefront.squeeze().shape != self.amplitude.squeeze().shape :
