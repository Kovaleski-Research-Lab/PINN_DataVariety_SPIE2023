...
def gen_train_splits():
    ...
    #Save them to disk
    torch.save((single_sample_0, single_target_0), 'new_splits/MNIST/mnist_single0.split')
    torch.save((single_sample_1, single_target_1), 'new_splits/MNIST/mnist_single1.split')
    torch.save((one_per_class_samples, one_per_class_targets), 'new_splits/MNIST/mnist_1perClass.split')
    torch.save((ten_per_class_samples, ten_per_class_targets), 'new_splits/MNIST/mnist_10perClass.split')
    torch.save((hundred_per_class_samples, hundred_per_class_targets), 'new_splits/MNIST/mnist_100perClass.split')   
    torch.save((thousand_per_class_samples, thousand_per_class_targets), 'new_splits/MNISTmnist_1000perClass.split')   

    torch.save((hundred_ones_samples, hundred_ones_targets), 'new_splits/MNIST/mnist_100_1.split')   
    torch.save((hundred_eights_samples, hundred_eights_targets), 'new_splits/MNIST/mnist_100_8.split')   

    torch.save((ten_ones_samples, ten_ones_targets), 'new_splits/MNIST/mnist_10_1.split')   
    torch.save((ten_eights_samples, ten_eights_targets), 'new_splits/MNIST/mnist_10_8.split')   



def gen_test_val_splits():

    dataset = datasets.MNIST('', train=False, download=True, transform=transforms.ToTensor())
    x = np.array(dataset.data)
    y = np.array(dataset.targets, dtype=np.int64)
    num_samples = x.shape[0]
    ...
    torch.save((val_samples, val_targets), 'new_splits/MNIST/valid.split')
    torch.save((test_samples, test_targets), 'new_splits/MNIST/test.split')
